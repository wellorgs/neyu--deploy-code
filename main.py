...
@app.route("/")
def index():
    return "✅ AI Therapy App is running!", 200
...
try:
    print("🧠 Registering profile_bp")
    app.register_blueprint(profile_bp)

    print("📊 Registering insights_bp")
    app.register_blueprint(insights_bp)

    print("🙏 Registering gratitude_bp")
    app.register_blueprint(gratitude_bp)

    print("📈 Registering combined_bp")
    app.register_blueprint(combined_bp)

    print("🛠️ Registering coping_techniques_bp")
    app.register_blueprint(coping_techniques_bp)

    print("🔄 Registering combined_progress_bp")
    app.register_blueprint(combined_progress_bp)
...
def get_bot_conversations(bots_json, bot_name):
    """
    Returns only the conversations for the bot with name `bot_name`.
    """
    for bot in bots_json.get("bots", []):
        if bot.get("name") == bot_name:
            return bot.get("conversations", [])
    return []  # fallback if bot not found
...
json_path = os.path.join(os.path.dirname(__file__), 'merged_bots_clean.json')
...
def stream():
    try:
        ...
        user_data = {
            ...
            "botName": request.args.get("botName", "Ava"),
        ...
        current_bot = user_data["botName"]
        session_id = f"{user_id}_{current_bot}"
        conversations = get_bot_conversations(data, current_bot)
        # 2️⃣ Find the best-matched response
        reply = find_best_response(conversations, user_msg)
        if not reply:
            reply = "I'm not sure how to respond to that yet. Could you try rephrasing?"

        # 3️⃣ Define the streaming generator
        def real_stream():
            try:
                ...
                # 4️⃣ Save to Firestore (like newstream)
                now = datetime.now(timezone.utc).isoformat()
                db.collection("sessions").document(session_id).set({
                    "user_id": user_id,
                    "bot_name": current_bot,
                ...
                yield "data: {\"error\": \"Streaming failed. Please try again.\"}\n\n"
