...
def compute_embedding(text):
    ...
    return sbert_model.encode(text, convert_to_tensor=False)
...
def find_best_response(bot_name, user_input):
    """
    ...
    keywords = {"name", "who", "tell", "about"}

    # Exact match
    for i in range(0, len(conversations) - 1, 2):
        conv_content_norm = normalize_text(conversations[i].get("content", ""))
        if user_input_norm == conv_content_norm:
            return conversations[i+1].get("content")
    if user_input_norm == conv_content_norm:
    # Return the assistant response (next message) ONLY if role is Assistant
        if conversations[i+1]['role'] == 'Assistant':
            return conversations[i+1].get("content")
        else:
            # log error or handle gracefully
            print(f"Warning: Expected Assistant role at index {i+1}, got {conversations[i+1]['role']}")
            return None
    # Fuzzy match fallback with keyword check
    best_score = 0.0
    best_reply = None
    for i in range(0, len(conversations) - 1, 2):
        conv_content_norm = normalize_text(conversations[i].get("content", ""))
        score = SequenceMatcher(None, user_input_norm, conv_content_norm).ratio()
        if score > best_score and (score >= 0.2 or (score >= 0.1 and contains_common_keywords(user_input_norm, conv_content_norm, keywords))):
            if conversations[i+1]['role'] == 'Assistant':
                best_score = score
...
for question in test_questions:
    response = find_best_response(bot_name, question)
    print(f"Q: {question}\nA: {response}\n")
